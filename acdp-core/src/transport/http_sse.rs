    ...
    async fn connect(&mut self) -> McpResult<()> {
        ...
        match test_response {
            Ok(_) => {
                ...
                // For legacy protocol servers that don't use dynamic session endpoints,
                // set a default session_id to avoid 5-second delays
                if self.session_id.is_none() {
                    self.session_id = Some("default".to_string());
                    tracing::info!("Set default session ID for legacy protocol server");
                }
