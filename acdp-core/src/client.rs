...
use crate::interceptor::{InterceptionMetadata, InterceptorManager, MessageDirection};
use crate::messages::{
...
impl McpClient {
    ...
    ///
    /// ```rust
    /// use acdp_probe_core::client::{McpClient, ClientConfig, DefaultNotificationHandler};
    /// use acdp_probe_core::transport::TransportConfig;
    ///
    /// # async fn example() -> acdp_probe_core::McpResult<()> {
    ...
    ///
    /// ```rust
    /// use acdp_probe_core::messages::Implementation;
    ///
    /// # async fn example(mut client: acdp_probe_core::client::McpClient) -> acdp_probe_core::McpResult<()> {
    ...
    /// Replace the internal interceptor manager (must be done before connect()).
    pub fn set_interceptor_manager(&mut self, manager: Arc<InterceptorManager>) {
        self.interceptor_manager = manager;
    }

    /// Attach session metadata used when invoking interceptors.
    ...
    fn interception_metadata(&self) -> InterceptionMetadata {
        match &self.session_id {
            Some(session) => InterceptionMetadata::default().with_session_id(session.clone()),
